...
const TextAnimation = ({ts,textItems,boxClass}) => {
 ...
 const c1=<div className={`${boxClass} mr-box flex flex-nowrap overflow-hidden whitespace-nowrap`}>
      <div className={`mr-wrapper flex flex-nowrap whitespace-nowrap gap-x-5 ${text}`} style={{transform:`translateX(-${number}%)`}}>
      {repeatedItems.map((item, indexmr)=>(
        <div key={indexmr} className="mr-item flex flex-nowrap items-center gap-x-4 whitespace-nowrap min-h-[74px] uppercase">
